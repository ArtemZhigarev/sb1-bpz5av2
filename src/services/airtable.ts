...
export const initAirtable = (tableName?: string) => {
  const { airtableToken, airtableBase, airtableTable, observationsTable } = useSettingsStore.getState();
  ...
  return base(tableName || airtableTable);
};
...
export const loadObservations = async (options: { offset?: string; pageSize?: number } = {}): Promise<LoadObservationsResult> => {
  ...
  try {
    const { observationsTable } = useSettingsStore.getState();
    const table = initAirtable(observationsTable);
    ...
    throw new Error('Failed to load observations: Unknown error');
  }
};
...
export const createObservation = async (observation: Omit<Observation, 'id'>): Promise<string> => {
  try {
    ...
    console.error('Failed to create observation in Airtable:', error);
    throw error;
  }
};
